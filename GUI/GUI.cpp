...
void GUI::PrintMessage(string msg , int currentLine ) const	//Prints a message on status bar
{
	if(currentLine == 0) ClearStatusBar();	//First clear the status bar

	pWind->SetPen(DARKRED);
	pWind->SetFont(18, BOLD , BY_NAME, "Arial");   
	pWind->DrawString(10, WindHeight - (int) (StatusBarHeight/1.5 + 15*currentLine), msg); // You may need to change these coordinates later 
	                                                                      // to be able to write multi-line
}
void GUI::PrintNumber(int num, int currentLine) const
{
	char number[10];
	itoa(num, number, 10);
	PrintMessage(number, currentLine);
}
//////////////////////////////////////////////////////////////////////////////////////////
...
void GUI::AddToDrawingList(Order* pOrd)
{
	...
	case WAIT:
		...
		break;
	case SRV:
		reg = SRV_REG;	//region of servicing orders
		break;
	case DONE:
		reg = DONE_REG;	//region of Done orders
		break;
	}
